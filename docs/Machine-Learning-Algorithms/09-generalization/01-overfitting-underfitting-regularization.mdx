...
| **Regularization** | Penalty term in cost function to shrink coefficients. |
| **L1 regularization (Lasso)** | "ell-one" | Penalty: $\lambda \sum |w|$. Shrinks some coefficients to exactly zero. |
| **L2 regularization (Ridge)** | Penalty: $\lambda \sum w^2$. Shrinks coefficients proportionally; none to zero. |
| **Elastic Net** | Combination of L1 and L2. Best of both. |
| **Regularisation strength λ** | Hyperparameter controlling penalty magnitude. Large \lambda -> strong penalty. |

:::tip 

If you only take one thing from this page
*   **Overfitting:** Model memorises noise (high training, low test accuracy).
*   **Underfitting:** Model is too simple (low training and test accuracy).
*   **Regularization:** Shrinks coefficients to prevent overfitting.
*   **L1 (Lasso):** Shrinks some coefficients to exactly zero (feature selection).
*   **L2 (Ridge):** Shrinks coefficients proportionally (all features kept).
...
**Loss = $\sum (y_i - \hat{y}_i)^2$** (e.g., squared error, regression)

Minimising this allows coefficients to grow large to fit noise.

### Cost function with regularization

**Loss + Penalty = $\sum (y_i - \hat{y}_i)^2 + \lambda \times \text{Penalty}(w)$**
...
<Tabs>
  <TabItem value="l2" label="L2 Regularization (Ridge)" default>
    <br/>
    
    **Penalty = $\sum w^2$ (sum of squared coefficients)**

    **Full loss = $\text{MSE} + \lambda \sum w^2$**

    *   **Proportional Shrinkage:** Shrinks all coefficients proportionally toward zero.
    *   **Keep All Features:** No coefficient reaches exactly zero (unless $\lambda \to \infty$).
    *   **Use Case:** Highly recommended when most features are useful and you want to prevent multicollinearity.
  </TabItem>
  <TabItem value="l1" label="L1 Regularization (Lasso)">
    <br/>
    
    **Penalty = $\sum |w|$ (sum of absolute values)**

    **Full loss = $\text{MSE} + \lambda \sum |w|$**

    *   **Feature Selection:** Shrinks weak coefficients to **exactly zero**, removing them from the model entirely.
    *   **Sparse Models:** Creates simpler, more interpretable models with fewer active parameters.
    *   **Use Case:** Best when you have high-dimensional features and suspect only a small subset of them carry real signal.
  </TabItem>
  <TabItem value="elastic" label="Elastic Net">
    <br/>
    
    **Penalty = $\alpha \sum |w| + (1-\alpha) \sum w^2$ (combination of both)**

    **Full loss = $\text{MSE} + \lambda [\alpha \sum |w| + (1-\alpha) \sum w^2]$**

    *   **Best of Both Worlds:** Combines the feature selection capability of L1 with the grouping stability of L2.
    *   **Balanced Control:** Tuned via hyperparameter $\alpha$ (which controls the relative weight of L1 vs L2).
    *   **Use Case:** Ideal for complex datasets where multiple features are correlated and you want to select groups of variables.
  </TabItem>
</Tabs>
...
Penalty = $0.5 \times (1.5^2 + 0.8^2 + 0.3^2) = 0.5 \times 2.98 = 1.49$
...
Penalty = $0.5 \times (|1.5| + |0.8| + |0.3|) = 0.5 \times 2.6 = 1.3$
...
Penalty = $0.5 \times [0.5 \times 2.6 + 0.5 \times 2.98] \approx 1.39$
...
| **L1 (Lasso)** | Penalty = $\lambda \sum |w|$. Shrinks some coefficients to zero. |
| **L2 (Ridge)** | Penalty = $\lambda \sum w^2$. Shrinks all coefficients proportionally. |
| **Elastic Net** | Penalty = $\alpha L_1 + (1-\alpha) L_2$. Combines both. |
...
**Next in this section:** [Regularization Strength and Solvers](./02-regularization-strength-and-solvers.md) - tuning lambda, solvers, hyperparameter search strategies.
