...
*   **dtypes Fail Silently:** A single invalid character (like a "?" or thousands separator ",") transforms columns into `object` strings. Always inspect `df.dtypes` immediately after every loading loop.
*   **Danger of Sentinels:** Numerical markers (such as `-1` or `999999`) bypass standard missingness parser checks. Only domain knowledge and `na_values` parameter declarations can capture them defensively.
