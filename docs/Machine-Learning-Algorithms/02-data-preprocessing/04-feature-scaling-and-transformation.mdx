...
export const ScalerSimulator = () => {
  ...
  return (
    <div style={{ padding: '20px', border: '1px solid var(--ifm-color-emphasis-200)', borderRadius: '8px', backgroundColor: 'rgba(148, 163, 184, 0.05)', margin: '20px 0' }}>
      <h4 style={{ margin: '0 0 10px 0' }}>Live Scaler Simulator</h4>
      ...
      <div style={{ fontSize: '12px', color: 'var(--ifm-color-emphasis-600)', fontStyle: 'italic', marginTop: '10px' }}>
        {scaler === 'minmax' && hasOutlier && "Notice how normal values (10, 20, 30) are squashed down to ~0.01. They are practically indistinguishable!"}
...
*   **Outliers crush MinMaxScaler:** A single outlier squashes legitimate data down to less than 1% of the scaled range; `RobustScaler` prevents this via median centering.
